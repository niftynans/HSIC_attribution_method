...
from xplique.attributions.global_sensitivity_analysis.replicated_designs import ScipyReplicatedSampler
from xplique.attributions.global_sensitivity_analysis.sobol_estimators import SobolEstimator
from xplique.attributions.global_sensitivity_analysis.gsa_attribution_method import PerturbationFunction
...
class HsicSampler(ScipyReplicatedSampler):
    """
...
class HsicQMCSampler(ScipyReplicatedSampler):
    """
    Pure Monte-Carlo sampler for HSIC.
    """
    def __init__(self, binary = False):
...
class HsicLHSSampler(ScipyReplicatedSampler):
    """
    Pure Monte-Carlo sampler for HSIC.
    """
    def __init__(self, binary = False):
...
class HsicSobolSampler(ScipyReplicatedSampler):
    """
    Pure Monte-Carlo sampler for HSIC.
    """
    def __init__(self, binary = False): 
    ...
    def __call__(self, dimension, nb_design): 
        sampler = self.qmc.Sobol(dimension*2)
        print("*** Sobol Sampler ***")
        masks = sampler.random(nb_design//2).astype(np.float32)
        if self.binary:
          masks= np.round(masks)
...
class HsicHaltonSampler(ScipyReplicatedSampler):
    """
    Pure Monte-Carlo sampler for HSIC.
    """
    def __init__(self, binary = False):
    ...
    def __call__(self, dimension, nb_design):
        sampler = self.qmc.Halton(dimension*2)
        print("*** Halton Sampler ***")
